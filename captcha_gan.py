...
def generator(input_dim,alpha=0.2):
    ...
    model.add(Dense(input_dim=input_dim, units=4*4*512))
...
# Defining a combination of Generator and Discriminator 
    ...
    for epoch in range(epochs):
        ...
        for index in range(int(X_train.shape[0]/batch_size)):
            ...
            if index % 20 == 0:
                ...
                # Images converted back to be awithin 0 to 255  
                #mage = image*127.5+127.5
            print(f'shapes-----{image_batch.shape},{generated_images.shape}')
            print(f'More shapes-----{type(image_batch)},{type(np.array([1 - smooth_coef]*batch_size))}')
            X = np.concatenate((image_batch, generated_images))
            d1 = d.train_on_batch(image_batch,np.array([1 - smooth_coef]*batch_size))
            d2 = d.train_on_batch(generated_images,np.array([0]*batch_size))
            ...
            g_loss = g_d.train_on_batch(noise, np.array([1] * batch_size))
